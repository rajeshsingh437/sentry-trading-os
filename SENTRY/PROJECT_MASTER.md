...
### 13.2 Real-Time In-Trade Tagging & Corrective Nudges — BUILT (v2, see Work Log)
- **v1:** a floating quick-tag button (bottom-left, visible on every screen, not buried in one view) opens a popover with one-tap buttons — FOL, SM, TTR, NTD, SOH, LPT, PA Exit — each defined in `TAGS_GLOSSARY.md`. Tapping one logs it with a timestamp (optional short note) and shows a dismissible nudge with that tag's specific corrective reminder.
- **v2 (this build): tags are now editable, and mapped tags feed the ABC Game / bell curve / Psychology score directly.** A "+ Add / manage tags" link inside the popover switches it into a management view: add a brand-new tag (code, label, nudge text, optional ABC mapping), or delete any existing one (including the 7 seeded defaults — full control, not just additions). Tag definitions persist in `localStorage` (`tos_tag_defs_v1`) and survive restarts like everything else.
- **ABC mapping, exactly as asked:** a tag can optionally carry a Mental/Tactical + A/B/C bucket. Logging a mapped tag (e.g. FOL → Tactical C) pushes a synthetic entry into the *same* `state.abcChecked` array the manual A/B/C Game Self-Check grid uses — so `computeAbcVerdict()`, `computePsychologyScore()`, the Alerts banner, and the bell curve (`state.abcHistory`, archived at day rollover) all update automatically, immediately, with zero duplicate logic. This is deliberate reuse of the existing system, not a parallel one. Context tags (TTR, NTD, PA Exit) are left unmapped on purpose — they describe the market or a method, not Rajesh's behavior, so they shouldn't move a game score.
- **Default ABC mapping used (my best-guess, easily changed in-app):** FOL → Tactical C ("Cutting trades too soon out of fear of loss" is the closest existing item), SM → Mental C ("Distracted"), SOH → Tactical A ("Patiently waiting for the default A+ setup only"), LPT → Tactical C ("No clear read on the default play, leading to a low-probability trade"). Flagged clearly — these are judgment calls, not something Rajesh stated; change any of them via "+ Add / manage tags" if they don't feel right.
- Usable both during a trade and before one, since it's available from any screen at any time.
- The Live Tag Log panel (Session Checklists) now shows an ABC column alongside time/tag/note, so it's visible at a glance which entries moved a game score and which didn't.
