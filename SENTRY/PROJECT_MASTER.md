...
Wire `view-journal`'s form to Python: save a Trade Object to SQLite through its lifecycle stages, enforcing the TLE cross-section validation rules before allowing a transition. Also where the structured `trades` table (Layer B) gets built, parsing journal entries out of the Layer A blob for fast analytics querying.
...
- **[Insert Date]** · **Phase 2 built: durable storage layer.** Added `storage.py` (SQLite `kv_store` table — generic key/value, mirrors every localStorage key the dashboard uses), a persistence patch at the top of `dashboard/index.html`'s `<head>` (mirrors every `localStorage.setItem`/`removeItem` into SQLite, no other lines of the existing dashboard touched), and a new `dashboard/boot.html` that hydrates localStorage from SQLite before handing off to the real dashboard. `main.py` updated: now opens `boot.html` instead of `index.html` directly, and `Api` gained `storage_get_all` / `storage_set_item` / `storage_remove_item`. **Architecture decision flagged:** built a generic key/value store first (Layer A) instead of the originally-planned rigid `trades` table, because it durably saves the *entire* app (journal, plans, checklists, capital plan, filters, everything) with zero risk to existing dashboard logic; the structured TLE-schema `trades` table (Layer B, needed for fast Analytics queries) is deferred into Phase 3 where it's needed anyway alongside Journal wiring. Section 5 and the Phase 2/3 SOP entries updated to match. `storage.py` logic verified independently (set/get/remove all confirmed working) before handoff. **Next:** Rajesh runs Phase 2 locally (steps below), confirms a trade/note logged in the dashboard survives closing and reopening the app, then commits. After that, **Phase 3 — Trade Journal wiring + structured trades table** begins. · **Flagged (carried over):** parallel React/TS repo should be paused/archived; free market-data source for VIX/USD-INR/Crude not yet chosen (Section 7); PyInstaller SmartScreen warning expected at Phase 11, not a bug.
